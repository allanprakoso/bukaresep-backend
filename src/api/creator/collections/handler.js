...
class CollectionsHandler {
    ...
    async getCollectionByIdHandler(request, h) {
        const { id: credentialId} = request.auth.credentials;
        const collection = await this.service.getCollectionById(request.params.id, credentialId);
    ...
    async getRecipesByCollectionIdHandler(request, h) {
        const { id: credentialId } = request.auth.credentials;
        const collection_id = request.params.id;
        const recipes = await this.service.getRecipesByCollectionId(credentialId,collection_id);
